...
export async function GET(req: Request) {
  ...
  const db = getDb()
  let query = db.from("school_analyses").select("id,slug,title,url,created_at").order("created_at", { ascending: false }).range(offset, offset + limit - 1)
...
export async function POST(req: Request) {
  ...
  const db = getDb()
  const { data, error } = await db.from("school_analyses").upsert({ slug, title, url }).select("id,slug,title,url,created_at").limit(1)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
...
export async function DELETE(req: Request) {
  ...
  const db = getDb()
  const { error } = await db.from("school_analyses").delete().in("id", ids)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
